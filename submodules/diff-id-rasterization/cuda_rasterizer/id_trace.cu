...
//  trace_count: [P],total traced number of each gaussian
// num_class: Maximum number of instances
{
	...
	float C[CHANNELS] = { 0 };//id of this pixel
	for (int ch = 0; ch < CHANNELS; ch++) {
    	C[ch] = id_masks[ch * H * W + pix_id];
  	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float nor_o = collected_opacity[j];
			float opa = nor_o.w;
			...
			atomicAdd(weights + (collected_id[j] * num_class + C[0]), 1);
			T = test_T;
